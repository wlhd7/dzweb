...
- [x] Task: 迁移图片工具类逻辑。 3c5f6af
    - [x] Write tests for path resolution in `tests/test_image_utils.py`.
    - [x] 修改 `dzweb/utils/image.py` 使其通过测试。
- [x] Task: 更新产品管理模块的上传逻辑。 3c5f6af
    - [x] Write tests for product image creation and deletion in `tests/test_product_api.py`.
    - [x] 更新 `dzweb/routes/admin.py` 或 `dzweb/routes/product.py` 中的相关路由逻辑。
- [x] Task: 更新模板中的图片引用路径（如果之前使用的是 `url_for('uploaded_file', ...)`，可能需要调整）。 3c5f6af
- [~] Task: Conductor - User Manual Verification 'Code Logic Migration' (Protocol in workflow.md)

## Phase 4: 物理迁移与最终验证 (Data Migration & Final Verification) [checkpoint: bc0d657]
- [x] Task: 执行物理迁移：将 `instance/uploads/` 下的所有内容移动到 `dzweb/static/uploads/`。
- [x] Task: 验证网站现有产品、案例图片的显示是否正常。

...
- [ ] Task: 编写多语言 `hreflang` 和 `canonical` 标签的单元测试
- [ ] Task: 在 `base.html` 中实现 `hreflang` 自动关联逻辑
- [ ] Task: 在 `base.html` 中实现 `canonical` 标签逻辑
- [ ] Task: 更新全局基础 TDK 默认值（中/英/日）并在 `base.html` 中应用
